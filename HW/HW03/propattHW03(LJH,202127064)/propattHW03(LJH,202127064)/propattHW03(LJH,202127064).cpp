...
int main() {
    ...
    std::cout << "이름 : 이지호\n\n";
    while (keycode >= 0 && keycode <= 64) // keycode가 0부터 64사이 값일때만 반복 실행
    {
        ...
        for (int i = 0; i < binaryString.size(); ++i) {
            if (binaryString[i] == '1') {
                if (i == 7)
                    std::cout << "\x1b[37m"; // 흰색으로 출력 (2진수 1의 자리)
                else if (i == 6)
                    std::cout << "\x1b[31m"; // 빨강색으로 출력 (2진수 10의 자리)
                else if (i == 5)
                    std::cout << "\x1b[32m"; // 녹색으로 출력 (2진수 100의 자리)
                else if (i == 4)
                    std::cout << "\x1b[33m"; // 노란색으로 출력 (2진수 1000의 자리)
                else if (i == 3)
                    std::cout << "\x1b[36m"; // 청록색으로 출력 (2진수 10000의 자리)
                else if (i == 2)
                    std::cout << "\x1b[35m"; // 보라색으로 출력 (2진수 100000의 자리)
            }
